...
def atualizar_mapa():
    ...
    if resultado:
        latitude, longitude, nomeDispositivo = resultado
        centro_lat, centro_lon = latitude, longitude
        mymap = folium.Map(location=[latitude, longitude], zoom_start=20)
        folium.Marker(location=[latitude, longitude], popup=f"{nomeDispositivo}").add_to(mymap)
        raio = 50
        folium.Circle(
        ...
        # if distancia > raio:
        #     print(f"O pet {nomeDispositivo} saiu do raio de {raio} metros!")
        # else:
        #     print(f"O pet {nomeDispositivo} está dentro do raio :)"s)
